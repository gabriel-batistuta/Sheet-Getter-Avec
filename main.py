...
def set_only_values_between(last_date_updated=None):
    ...
    if initial_date_value == last_date_updated and final_date_value == atual_date:
        try:
            ...
            print('error in click search button')
            return None
        print(f"Esperando carregar dados...")
        sleep(60)
        return True
        # first_value_div = WebDriverWait(driver, 40).until(
...
if __name__ == '__main__':
    ...
    if platform.system() == 'Linux':
        chrome_driver_path = Service('./chromedriver_LINUX')
        driver = webdriver.Chrome(service=chrome_driver_path, options=chrome_options)
    else:
        chrome_driver_path = Service('./chromedriver_131_WINDOWS.exe')
        driver = webdriver.Chrome(service=chrome_driver_path, options=chrome_options)
